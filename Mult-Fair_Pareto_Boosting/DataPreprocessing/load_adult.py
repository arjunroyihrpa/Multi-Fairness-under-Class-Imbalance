...
def load_adult():
	...
	CONT_VARIABLES = [ "Capital-gain","Capital-loss","Hours-per-week","age"] # continuous features, will need to be handled separately from categorical features, categorical features will be encoded using one-hot
	CLASS_FEATURE = "Class-label" # the decision variable
	SENSITIVE_ATTRS = ["race", "sex"]#["Maritial-status","race", "sex","age"]#["race", "sex"]#
	...
	for attr in FEATURES_CLASSIFICATION:
		...
		if attr=="Maritial-status" and "Maritial-status" in SENSITIVE_ATTRS:
			vals=[0 if v in ["' Married-AF-spouse'","' Married-civ-spouse'","' Married-spouse-absent'"] else 1 for v in vals] 
	...
	return X, y, [feature_names.index(SENSITIVE_ATTRS[i]) for i in range(len(SENSITIVE_ATTRS))],p_group, x_control,feature_names
               #feature_names.index(SENSITIVE_ATTRS[2]),feature_names.index(SENSITIVE_ATTRS[3])], p_group, x_control,feature_names
